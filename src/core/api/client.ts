...
// Indique qu'un login est en cours — bloque refresh/logout pendant authApi.me() post-login.
...
// Protege le flux de login contre les 401 interceptes pendant authApi.me() post-login.
  ...
  async (error) => {
    ...
    if (
      ...
      try {
        ...
        if (!logoutFired && !appInitializing && !loginInProgress) {
          logoutFired = true;
