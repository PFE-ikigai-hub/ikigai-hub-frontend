...
export function AppShell() {
  ...
  const handleNotificationClick = async (id: number, routePath?: string | null) => {
    ...
    setIsNotificationsOpen(false);
    if (routePath) {
      sessionStorage.setItem("ikigai:navSplash", "1");
      navigate(routePath);
    }
  };
                ...
                {menu.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative w-full aspect-square flex items-center justify-center rounded-xl transition-all duration-200 group ${isActive
                    ...
                    >
                      ...
                      <span className="relative z-[10000] flex items-center justify-center">
                        <Icon
                          className={`w-5 h-5 transition-colors shrink-0 ${isActive
                            ? "text-stone-900 dark:text-white"
                            : "text-stone-500 dark:text-stone-400 group-hover:text-stone-800 dark:group-hover:text-stone-200"
                            }`}
                        />
                        <div
                          className={`absolute px-2 py-1 bg-black dark:bg-white text-white dark:text-black text-[10px] font-semibold rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap shadow-lg z-[10001] pointer-events-none ring-1 ring-black dark:ring-white uppercase tracking-wide ${isArabic
                        ...
                        >
                          {t(item.labelKey)}
                          <div
                            className={`absolute top-1/2 -translate-y-1/2 border-[6px] border-transparent ${isArabic
            ...
            <div className="pt-4 mt-4">
              {showNotifications && (
                <div className="relative mb-2" ref={notificationsRef}>
                  {isNotificationsOpen && (
                    <div
                      className={`absolute bottom-[calc(100%+12px)] w-80 max-h-[380px] bg-white dark:bg-stone-900 rounded-xl shadow-2xl shadow-stone-300/40 dark:shadow-black/70 border border-stone-200 dark:border-stone-700/60 overflow-hidden py-1.5 z-[12050] isolate ${isArabic ? "right-0 text-right" : "left-0"
                        }`}
                        ...
                        {notifications.length === 0 ? (
                          ...
                          notifications.map((item) => (
                            <button
                              ...
                              className={`w-full text-left px-3 py-2.5 rounded-lg transition-colors ${item.lu
              ...
              <div className="relative" ref={profileRef}>
                {isProfileExpanded && (
                  <div
                    className={`absolute bottom-[calc(100%+12px)] w-56 bg-white dark:bg-stone-900 rounded-xl shadow-2xl shadow-stone-200/40 dark:shadow-black/70 border border-stone-200 dark:border-stone-700/60 overflow-hidden py-1.5 z-[12040] ${isArabic ? "right-0 text-right" : "left-0"
                      }`}
                    dir={isArabic ? "rtl" : "ltr"}
                  >
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-3 py-2 text-[13px] text-stone-700 dark:text-stone-300 hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors ${isArabic ? "flex-row-reverse justify-end" : ""
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-3 py-2 text-[13px] text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors ${isArabic ? "flex-row-reverse justify-end" : ""
                        }`}
                    >
