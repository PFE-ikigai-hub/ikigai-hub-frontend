...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  const getBackgroundStyle = (_color: string): React.CSSProperties => {
    return { background: "var(--ikg-mono-surface-gradient)" };
  };
    ...
    <div className={`grid gap-[5em] grid-cols-2 md:grid-cols-3 mx-auto py-[3em] overflow-visible ${className || ""}`}>
      {items.map((item, index) => (
        ...
        >
          ...
          >
            <span className="m-auto w-[1.5em] h-[1.5em] flex items-center justify-center text-white dark:text-black" aria-hidden="true">
              {item.icon}
            </span>
          </span>
