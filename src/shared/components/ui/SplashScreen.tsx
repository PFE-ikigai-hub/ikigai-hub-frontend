...
export function SplashScreen({ isLoading, children, showWelcome = true, isNavSplash = false }: SplashScreenProps) {
  const { t } = useI18n();
      ...
      {showSplash && (
        isNavSplash ? (
          <motion.div
            ...
            className="fixed inset-0 z-50 flex items-center justify-center bg-white dark:bg-[#0a0a0b] transition-colors duration-300 overflow-hidden"
          >
            <div className="flex flex-col items-center justify-center">
              <div className="w-12 h-12 rounded-full border-4 border-stone-200 dark:border-stone-800/30 border-t-stone-900 dark:border-t-white animate-spin" />
            </div>
          </motion.div>
        ) : (
          <motion.div
