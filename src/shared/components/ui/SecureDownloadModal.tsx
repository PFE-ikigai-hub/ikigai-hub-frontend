...
type SecureDownloadModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (payload: DownloadConfirmationPayload) => Promise<void>;
};

export function SecureDownloadModal({ isOpen, onClose, onConfirm }: SecureDownloadModalProps) {
  const { t } = useI18n();
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      await onConfirm({ password, confirmA, confirmB });
      onClose();
    } catch (err: any) {
            ...
            >
              <div className="flex items-center justify-between px-6 pt-6 pb-5 border-b border-stone-100 dark:border-stone-800/60">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/40 flex items-center justify-center">
                    <ShieldAlert className="w-5 h-5 text-amber-600 dark:text-amber-400" />
                  </div>
                  <div>
                    <h2 className="text-lg tracking-tight text-stone-900 dark:text-white">{t("secure_download_title")}</h2>
                    <p className="text-xs text-stone-400 dark:text-stone-500 mt-0.5">{t("secure_download_message")}</p>
                  </div>
                </div>
                <button
                  onClick={onClose}
              ...
              <div className="p-6 space-y-4">
                <label className="flex items-start gap-2 text-sm text-stone-700 dark:text-stone-300">
                  <input
                    ...
                    className="mt-0.5"
                  />
                  <span>{t("download_strong_check_1")}</span>
                </label>
                <label className="flex items-start gap-2 text-sm text-stone-700 dark:text-stone-300">
                  <input
                    type="checkbox"
                    ...
                    className="mt-0.5"
                  />
                  <span>{t("download_strong_check_2")}</span>
                </label>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={!canSubmit}
                    className="flex-1 px-4 py-3 bg-stone-900 text-white dark:bg-stone-100 dark:text-stone-900 rounded-xl text-sm font-medium hover:bg-stone-800 dark:hover:bg-stone-200 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    <Download className="w-4 h-4" />
                    {submitting ? t("loading") : t("download_confirm")}
                  </button>
                </div>
              </div>
            </motion.div>
