...
function shouldHideEmployeeProjectAccessMessage(message: unknown) {
...
export function EmployeeFeedbackDetailPage() {
  ...
  const [editingNoteInterne, setEditingNoteInterne] = useState(false);
  ...
  useEffect(() => {
    setNoteInterneDraft(currentVersion?.noteInterne ?? '');
    setEditingNoteInterne(false);
  }, [currentVersion?.id]);
  ...
  const canDeleteVersion =
    ...
    !!livrable &&
    !!currentVersion &&
    Number(user.id) === livrable.deposeParId &&
    livrable.statut !== 'VALIDE' &&
    currentVersion.statut !== 'VALIDATED' &&
    (currentVersion.commentaires?.length ?? 0) === 0 &&
    (currentVersion.annotations?.length ?? 0) === 0 &&
    (currentVersion.validations?.length ?? 0) === 0;

  const handleDeleteVersion = () => {
    if (!canDeleteVersion || !currentVersion || !livrable || !user) return;
    setDeleteVersionModalOpen(true);
  };
  
  const executeDeleteVersion = async (_adminPassword?: string) => {
    if (!currentVersion || !livrable || !user) return;
    try {
      ...
      if (shouldHideEmployeeProjectAccessMessage(backendMessage)) return;
      toast.error(typeof backendMessage === "string" ? backendMessage : t("common.error"));
      throw e;
    }
  };

  const saveNoteInterne = async () => {
    if (!currentVersion?.id) return;
    try {
      setSavingNoteInterne(true);
      const updated = await versionsApi.updateNoteInterne(currentVersion.id, noteInterneDraft.trim() || null);
      setVersions((prev) => prev.map((v) => (v.id === updated.id ? { ...v, noteInterne: updated.noteInterne } : v)));
      setEditingNoteInterne(false);
      toast.success(t("review.internalNoteSaved"));
    } catch (e: any) {
      const backendMessage = e?.response?.data?.message;
      toast.error(typeof backendMessage === "string" ? backendMessage : t("common.error"));
    } finally {
      setSavingNoteInterne(false);
    }
  };
                  ...
                  <div className="text-xs text-stone-500 dark:text-stone-400 flex flex-col gap-2">
                    ...
                    {currentVersion.uploadedByName && (
                      <div className="text-[11px]">
                        {t("review.uploadedBy")}: <span className="font-medium text-stone-700 dark:text-stone-300">{currentVersion.uploadedByName}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="max-w-[520px]">
                  {editingNoteInterne ? (
                    <div className="space-y-2">
                      <textarea
                        value={noteInterneDraft}
                        onChange={(e) => setNoteInterneDraft(e.target.value)}
                        rows={3}
                        className="w-full text-xs text-stone-700 dark:text-stone-300 bg-stone-100/80 dark:bg-stone-800/60 border border-stone-200/70 dark:border-stone-700/60 rounded-md px-2 py-1 resize-none"
                        placeholder={t("review.internalNotePlaceholder")}
                      />
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={saveNoteInterne}
                          disabled={savingNoteInterne}
                          className="px-2 py-1 text-[11px] rounded-md bg-stone-900 text-white dark:bg-white dark:text-stone-900 disabled:opacity-50"
                        >
                          {savingNoteInterne ? "..." : t("common.save")}
                        </button>
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="px-2 py-1 text-[11px] rounded-md border border-stone-300 dark:border-stone-700 text-stone-600 dark:text-stone-300"
                        >
                          {t("common.cancel")}
                        </button>
                      </div>
                    ...
                    <button
                      type="button"
                      onClick={() => setEditingNoteInterne(true)}
                      className="w-full text-left text-xs text-stone-700 dark:text-stone-300 bg-stone-100/80 dark:bg-stone-800/60 border border-stone-200/70 dark:border-stone-700/60 rounded-md px-2 py-1 hover:bg-stone-200/80 dark:hover:bg-stone-700/60 transition-colors"
                    >
                      <span className="font-semibold">{t("review.internalNote")}:</span>{" "}
                      {currentVersion?.noteInterne?.trim() ? currentVersion.noteInterne : t("review.internalNoteEmpty")}
                    </button>
                  )}
                </div>

                <div className="flex flex-wrap gap-2">
                  {canDeleteVersion && currentVersion && (
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      onClick={handleDeleteVersion}
