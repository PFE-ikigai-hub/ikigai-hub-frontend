...
import { deliverablesApi, projectsApi, versionsApi, getApiBaseUrl } from "@/core/api/client";
...
function DeliverablesView({ viewType, titleKey }: DeliverablesViewProps) {
  ...
  const [layout, setLayoutState] = useState<"grid" | "list">(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('ikigai:clientLayout');
  ...
  const setLayout = useCallback((newLayout: "grid" | "list") => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('ikigai:clientLayout', newLayout);
    }
  }, []);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        versionsByDeliverableId = new Map();
      }

      const projectStatusById = new Map<number, ProjectStatus>();
      (projectsRes.content ?? []).forEach((p: ApiProject) => {
        projectStatusById.set(p.id, p.statut);
      });

      const enriched = deliverables.map((item) => {
        const normalized = normalizeVersions(versionsByDeliverableId.get(item.id) ?? []);

        let latest = getLatestVersion(normalized) ?? normalized[0] ?? (item.latestVersionId ? {
  ...
  const filteredItems = items.filter((item) => {
    if (viewType === "mes-projets") {
      if (item.projectStatus === "ARCHIVE") return false;
    }
                ...
                {paginatedItems.map((item) => (
                  <motion.div key={item.id} variants={itemVariants}>
                    <DeliverableCard
                      ...
                      layout="grid"
                      onClick={(id) => navigate(`/client/review/${id}`)}
                    />
                  </motion.div>
                ))}
              </motion.div>
