...
function PostLoginSplashGate({ role }: { role: UserRole | null }) {
  ...
  let isNavSplash = false;
  try {
    shouldShow = sessionStorage.getItem("ikigai:postLoginSplash") === "1";
    isNavSplash = sessionStorage.getItem("ikigai:navSplash") === "1";
    if (shouldShow) {
  ...
  if (!shouldShow && !isNavSplash) return <RoleRedirect role={role} />;

  return (
    <SplashScreen isLoading={false} showWelcome={shouldShow && !isNavSplash} isNavSplash={isNavSplash}>
